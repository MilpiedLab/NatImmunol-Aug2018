# #########################################
# all my own functions used for this paper
# #########################################
library(gplots)
library(ggplot2)
library(gridExtra)
...
# #####################################################
# #####################################################
run_tsne_function <- function( raw_data_for_tsne ,perplexity_value=40){
...
# #####################################################
# #####################################################
# pca function which generate beautiful plots
# #####################################################
# #####################################################
# #####################################################
pca_function <- function( raw_data_for_pca, PlotHist = FALSE, VectorToColorPCA = NULL, NameOfVectorToColor = "", 
                          dim_all=FALSE, dim_x="PC1", dim_y="PC2", bool_facet_wrap=FALSE, VectorFacetWrap=NA, 
                          title="",bool_scalePCA=FALSE, bool_centerPCA=TRUE, bool_plot=TRUE){
  limits_allFigs=c(0,27)
  ...
  # special case if we want to color the pca by gene expression (continuous variable)
  if(NameOfVectorToColor %in% colnames(raw_data_for_pca) ) {
  ...
  return(result__pca) #return(list(pca_info = result__pca, genes_information = names_best_on_axis_all_axes))
}
...
# #####################################################
# #####################################################
pseudotimeDDRTree_function <- function(raw_data_for_pseudotimeDDRTree,GENES_TO_ANALYSE, 
                                       cell_attribute_colorBy="SimpleSortPheno", FL_specific_Sample=NA, 
                                       cell_attribute_facetwrapBy=NA ,bool_facet_wrap=FALSE, 
                                       DDRTree_max_components=2, cellsAnalyse="unknown"){
  #DDRTree_max_components=2
  
  phenotypes_to_study=sort(unique(raw_data_for_pseudotimeDDRTree$SimpleSortPheno))
  ...
  if(TRUE){
    ...
    raw_data_transposed=t(raw_data_for_pseudotimeDDRTree[, GENES_TO_ANALYSE])
    colnames(raw_data_transposed)=raw_data_for_pseudotimeDDRTree$UniqueCellID[1:ncol(raw_data_transposed)]
    
    # creation of a phenoData for the CellDateSet
    phenoData_data_frame=data.frame(SimpleSortPheno=raw_data_for_pseudotimeDDRTree$SimpleSortPheno,Sample=raw_data_for_pseudotimeDDRTree$Sample)
    for(gene in GENES_TO_ANALYSE){
      phenoData_data_frame[[gene]]=as.vector(raw_data_transposed[gene,])
    }
    phenoData_data_frame[["SimplePheno"]]=raw_data_for_pseudotimeDDRTree$SimpleSortPheno
    phenoData_data_frame[["SampleSimplePheno"]]=raw_data_for_pseudotimeDDRTree$Sample.SimpleSortPheno
    phenoData_data_frame[["IndexedPheno"]]=raw_data_for_pseudotimeDDRTree$IndexedPheno
    phenoData_data_frame[["Plate"]]=raw_data_for_pseudotimeDDRTree$Plate
    phenoData_data_frame[["CellNumber"]]=raw_data_for_pseudotimeDDRTree$CellNumber
  ...
  # make the titles of the plots:
  string_simplepheno=paste(phenotypes_to_study,collapse=" & ")
  #ggplot_DDRTree_title=paste("DDRTree Pseudotime on ", string_simplepheno, " colored by ", cell_attribute_colorBy, sep="")
  
  # ggplot_title and pheatmap title
  if(any(is.na(FL_specific_Sample))){
    ggplot_DDRTree_title=paste("DDRTree Pseudotime on ",string_simplepheno,"\n All Samples." ,sep="")
  } else{
    ggplot_DDRTree_title=paste("DDRTree Pseudotime on ",string_simplepheno,"\n Samples for FL: ",paste(sort(unique(FL_specific_Sample)),collapse=" ") ,sep="")
  }
  ...
  if(DDRTree_max_components==3 & cellsAnalyse=="GC"){
    tree_MST_niaki_DDRTree=orderCells(tree_MST_niaki_DDRTree, root_state=11)
  }
  ...
  for(cell_attribute_colorBy_value in cell_attribute_colorBy){
    ...
    ggplot_cell_trajectory_multi<-plot_cell_trajectory(tree_MST_niaki_DDRTree,cell_size=1, show_branch_points = FALSE, color_by=cell_attribute_colorBy_value)+ facet_wrap(c(cell_attribute_facetwrapBy))#, color_by="SortPheno") #, markers="MKI67")
...
# #####################################################
# #####################################################
# correlation between two pseudotimes
# #####################################################
# #####################################################
# #####################################################
correlation_pseudotime_function <- function (pseudotime_axis1, pseudotime_axis2, name_axis1="Pseudotime 1", name_axis2="Pseudotime 2", FigureFileSource=NULL, FigureName=NULL,FigureRDataSource=NULL){
  #pseudotime_axis1 = pseudotimeGC_PB_Mem_onlyPB
  #pseudotime_axis2 = pseudotimeDDRTree_PB
  
  #pseudotime_axis1 & pseudotime_axis2 should have the UniqueCellID as names of the vector
  
  #reorder of the values between the 2 axis, thanks to the UniqueCellID
  pseudotime_axis2_ordered=c()
  for (i in 1:length(pseudotime_axis1)){
    for(j in 1:length(pseudotime_axis2)){
      if(identical(names(pseudotime_axis1)[i],names(pseudotime_axis2)[j])){
        pseudotime_axis2_ordered=c(pseudotime_axis2_ordered,pseudotime_axis2[[j]])
      }
    }
  }
  
  data_ggplot_df=data.frame(PseudotimeAxis1=as.vector(pseudotime_axis1), PseudotimeAxis2=as.vector(pseudotime_axis2_ordered))
  
  reg_lin = lm(data_ggplot_df$PseudotimeAxis2 ~ data_ggplot_df$PseudotimeAxis1)
  reg_lin_sd = summary( reg_lin)$coefficients[1,2]
  pearson_cor = signif( cor( data_ggplot_df$PseudotimeAxis2, data_ggplot_df$PseudotimeAxis1, method="pearson", use="complete"), digits=3)
  cor_p_value=signif(cor.test( data_ggplot_df$PseudotimeAxis2, data_ggplot_df$PseudotimeAxis1)$p.value, digits=3)
  title_plot=paste("Correlation between the two pseudotimes\n","Correlation Pearson=",pearson_cor,"\n p-value =",cor_p_value, sep=" ")
  
  correlation_ggplot<-ggplot(data_ggplot_df, aes(PseudotimeAxis1, PseudotimeAxis2)) +
    geom_point(size=0.7) +#  facet_wrap(~State) +
    ggtitle(title_plot) +
    labs(x=name_axis1, y=name_axis2) +
    geom_abline(aes(intercept=reg_lin$coefficients[1], slope=reg_lin$coefficients[2]), colour="blue") +
    geom_abline(aes(intercept=reg_lin$coefficients[1] - 1.96*reg_lin_sd, slope=reg_lin$coefficients[2]), lty=3, colour="blue") +
    geom_abline(aes(intercept=reg_lin$coefficients[1] + 1.96*reg_lin_sd, slope=reg_lin$coefficients[2]), lty=3, colour="blue") 
  return(correlation_ggplot)
}
...
# #####################################################
# #####################################################
# function allowing to test the weighted voting model on a test dataset
# #####################################################
# #####################################################
# #####################################################
test_model <- function( index_list, both_zone_data, pgc, b, sense){
  
  good_vote = 0
  bad_vote = 0
  bad_vote_index_set = vector()
  bad_vote_strength_set = vector()
  vote_strength_set = vector()
  for( index in index_list){
    ...
    for( gene_name in WEIGHTED_VOTING_MODEL_GENES){
      if( !is.na( pgc[gene_name]) & !is.na( b[[ gene_name]])){
        count = count + 1
        current_vote = pgc[[ gene_name]] * ( both_zone_data[ index, gene_name] - b[[ gene_name]])
        if( current_vote > 0){
          positive_vote = positive_vote + current_vote
        }else{
          negative_vote = negative_vote - current_vote
        }
      }
    }
    
    good=NA
    # we say that sense=TRUE if the excluded cell is from the 1st group
    # we say that sense=FALSE if the excluded cell is from the 2nd group
    if( positive_vote > negative_vote){
    ...
    }else if( positive_vote < negative_vote){
      if (sense){
        good = FALSE
      }else{
        good = TRUE
      ...
      # cat("\n", index, " -> ??? : ", positive_vote , " vs ", negative_vote, " (", count, " genes)", sep="")
    }
    
    if( !is.na( good)){
      if( good){
        good_vote = good_vote + 1
      }else{
        bad_vote = bad_vote + 1
        bad_vote_index_set = append( bad_vote_index_set, index)
        bad_vote_strength_set = append( bad_vote_strength_set, (positive_vote - negative_vote)/(positive_vote + negative_vote))
      }
      vote_strength_set = append( vote_strength_set, (positive_vote - negative_vote)/(positive_vote + negative_vote))
    }else{
      vote_strength_set = append( vote_strength_set, 0)
    }
    
  }
  
  return( list(good_number = good_vote, bad_number = bad_vote, bad_indexes = bad_vote_index_set, bad_strengths = bad_vote_strength_set, vote_strengths = vote_strength_set))
}
...
# #####################################################
# #####################################################
volcano_plot_LRT_function <- function(raw_data_for_volcano_plot, VectorToSplit){
...
function_coexpr_gene <- function(raw_data_df){
  coExpr_perCell_list=list()
  for(k in 1:nrow(raw_data_df)){
    coExpr_oneCell_vector=c()
    for(i in 1:(ncol(raw_data_df)-1)){
      for(j in (i+1):ncol(raw_data_df)){
        geneI = raw_data_df[k,i]
        geneJ = raw_data_df[k,j]
        coExpr_category=NA
        if(geneI==0 && geneJ==0){
          coExpr_category="A"
        } else if(geneI==0){
        ...
        coExpr_oneCell_vector = c(coExpr_oneCell_vector, coExpr_category)
      }
    }
    coExpr_perCell_list[[ rownames(raw_data_df)[k] ]] = coExpr_oneCell_vector
  }
  
  return(coExpr_perCell_list)
}
